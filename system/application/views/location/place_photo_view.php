...
<html>
	...
	<script type="text/javascript" charset="utf-8">
		$(document).ready(function(){
			...
			$('#like_bth').click(function(){
			
				
				var userid = <?php echo $user_data['id']; ?>;
				var photoid = <?php echo $photo_data['id']; ?>;
				var placeid = <?php echo $place_data['id']; ?>;
				
				var url = '<?php echo site_url('location/like_photo/')?>';
				var like_data = {
					user_id : userid,
					photo_id : photoid,
					place_id : placeid,
				...
				$.post(url,like_data,function(data){
				
					if(data == 'LIKED'){
						$('#like_bth').text('Unlike');
					}else if(data == 'UNLIKED'){
						$('#like_bth').text('Like it');
					}
	...
	<div id="content" class="container">
		<div id="page">
			<div id="page_inner">
					<div id="photo_viewer_box">
						<img src=<?php echo site_url('/files/location_photo/'.$photo_data['image_path']);?>>
					</div>
					
					<?php if (isset($user_data)): ?>
						<div id="place_photo_action">
							<a href="#" id="like_bth"><?php echo $is_like ? "Unlike" : "Like" ; ?></a>
						</div>
					<?php endif ?>
